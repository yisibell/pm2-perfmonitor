...
const {
  sendCpuOverloadCpuProfileStart,
  sendCpuOverloadCpuProfileStop,
  sendZombieCpuProfileStart,
  sendZombieCpuProfileStop,
} = require('./message')
...
const enableNodeInspectorOnCpuOverload = parseBool(
  conf.enableNodeInspectorOnCpuOverload,
)
const enableNodeInspectorOnZombie = parseBool(conf.enableNodeInspectorOnZombie)
...
const processChecker = async () => {
  ...
  try {
    ...
    for (const app of apps) {
      ...
      if (
        enableNodeInspectorOnCpuOverload &&
        appCpuUsage >= cpuOverloadThreshold
      ) {
        await sendCpuOverloadCpuProfileStart(pm_id)
        await sleepAsync(nodeInspectorSampleDuration * 1000)
        await sendCpuOverloadCpuProfileStop(pm_id)
      }
      ...
      if (ZOMBIE_DETECTION && zombieDetected) {
        ...
        // 僵尸进程重启前，采集诊断数据（CPU profile + active handles/requests dump）
        if (enableNodeInspectorOnZombie) {
          try {
            logger(
              'info',
              `Collecting diagnostic data for zombie ${name} (pid: ${app.pid})...`,
            )
            await sendZombieCpuProfileStart(pm_id)
            await sleepAsync(nodeInspectorSampleDuration * 1000)
            await sendZombieCpuProfileStop(pm_id)
            logger(
              'info',
              `Diagnostic data collected for ${name} (pid: ${app.pid})`,
            )
          } catch (diagErr) {
            logger(
              'error',
              `Failed to collect diagnostic data for zombie ${name}: ${diagErr.message}`,
            )
          }
        }
